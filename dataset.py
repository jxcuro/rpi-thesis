...
# Version: 1.6.0 - Compact GUI Layout, Modal Feedback Popups
...
# GUI Globals (feedback_label removed)
...
# === File/State Utils ====
# =========================
# (Functions get_last_object_index, initialize_global_state, setup_project_directory remain the same as v1.5.1)
...
# === Hardware Setup ===
# =========================
# (initialize_hardware remains the same as v1.5.1)
...
# === LDC1101 Functions ===
# =========================
# (LDC functions remain the same as v1.5.1)
...
# === CSV Handling =========
# =========================
# (append_metadata remains the same as v1.5.1)
...
# === Sensor Reading (Avg) ===
# ============================
# (Sensor reading functions remain the same as v1.5.1)
...
def update_next_filename_display():
    ...
    try:
        ...
        next_filename_label.config(text=f"Next: {next_fn}") # Shortened prefix
...
def capture_and_save_data():
    """Captures image, reads sensors, updates counters, saves files, appends metadata. Uses messageboxes for feedback."""
    ...
    if not camera:
        messagebox.showerror("Camera Error", "Camera not available or failed to open.")
        return

    # Disable button immediately
    capture_button.config(state=tk.DISABLED)
    # No temporary feedback message needed here anymore

    ret, frame = camera.read()
    if not ret:
        messagebox.showerror("Capture Error", "Failed to capture photo from camera.")
        capture_button.config(state=tk.NORMAL) # Re-enable button on error
        return
    ...
    if is_new_object == 1:
        ...
        if object_counter == 0: object_counter = 1 # Handle first capture case
        shot_counter += 1
    ...
    if avg_voltage is not None:
        ...
        except Exception: current_mag_mT = None # Silent fail on calculation error
    ...
    try:
        ...
        messagebox.showerror("Save Error", f"Error saving photo to {image_save_path}:\n{e}")
        # Counter already incremented, maybe don't revert here for simplicity
        capture_button.config(state=tk.NORMAL)
    ...
    # --- Final Feedback and State Update ---
    if meta_success:
        messagebox.showinfo("Success", f"Data Added: {image_filename}")
        new_object_var.set(0) # Default to same object next time
    else:
        messagebox.showwarning("Metadata Error", f"Image '{image_filename}' saved,\nbut failed to write metadata to CSV.")

    update_next_filename_display()
    # Re-enable button after a short delay (optional, prevents rapid clicks)
    window.after(500, lambda: capture_button.config(state=tk.NORMAL))


def calibrate_sensors():
    """Calibrates sensors and shows results in a messagebox."""
    ...
    if hall_sensor:
        voltages = []
        # Maybe add a temporary "Calibrating..." label or disable button during this?
        # For now, just rely on the final messagebox.
    ...
    # Combine results for messagebox
    final_message = f"{hall_results}\n{ldc_results}"

    # Show appropriate messagebox
    if hall_error or ldc_error:
        messagebox.showwarning("Calibration Warning", final_message)
    elif not hall_sensor and not ldc_initialized:
         messagebox.showerror("Calibration Error", final_message) # Both N/A is an error state
    else:
        messagebox.showinfo("Calibration Complete", final_message)


# --- Camera/Sensor Update Functions ---
# (update_camera_feed, update_magnetism, update_ldc_reading remain the same as v1.5.1)
def update_camera_feed():
    ...
    if window: window.after(50, update_camera_feed) # Check if window exists
    ...
    if hall_sensor:
        if avg_voltage is not None:
            try:
                ...
                unit, value = ("mT", mag_mT) if abs(mag_mT) >= 1 else ("µT", mag_mT * 1000)
                display_text = f"{value:.2f} {unit}"
            except Exception: display_text = "Error"
        else: display_text = "Error" # Read error
...
# ======================
def setup_gui():
    """Sets up the Tkinter GUI elements with compact layout and no status bar."""
    ...
    window = tk.Tk()
    window.title("Sensor Data Acquisition Tool v1.6.0") # Updated version
    window.geometry("1100x680") # Adjusted default size slightly
    ...
    button_font = tkFont.Font(family="Helvetica", size=10, weight="bold") # Slightly smaller button font
    # feedback_font removed
    check_font = tkFont.Font(family="Helvetica", size=10)
    filename_display_font = tkFont.Font(family="Consolas", size=9)

    style.configure("TLabel", font=label_font, padding=2) # Reduced padding
    style.configure("TButton", font=button_font, padding=(8, 5)) # Reduced padding
    style.configure("TMenubutton", font=label_font, padding=4) # Reduced padding
    style.configure("TLabelframe", padding=8) # Reduced padding
    style.configure("TLabelframe.Label", font=tkFont.Font(family="Helvetica", size=12, weight="bold"))
    style.configure("Readout.TLabel", font=readout_font, padding=(5, 1)) # Reduced padding
    style.configure("TCheckbutton", font=check_font, padding=2) # Reduced padding
    style.configure("Filename.TLabel", font=filename_display_font, padding=(0, 2)) # Reduced padding

    # --- Main Content Frame (No Status Bar) ---
    main_frame = ttk.Frame(window, padding="10 10 10 10") # Reduced padding
    ...
    camera_label.grid(row=0, column=0, padx=(0, 10), pady=0, sticky="nsew") # Reduced padx
    ...
    readings_frame = ttk.Labelframe(controls_frame, text=" Sensor Readings ", padding="10 5 10 5") # Reduced padding
    readings_frame.grid(row=controls_row_idx, column=0, sticky="new", pady=(0, 5)) # Reduced bottom padding
    controls_row_idx += 1
    ...
    ttk.Label(readings_frame, text="LDC RP:").grid(row=1, column=0, sticky="w", padx=(0, 5), pady=(2,0)) # Reduced pady
    ldc_label = ttk.Label(readings_frame, text="Init...", style="Readout.TLabel", anchor="e")
    ldc_label.grid(row=1, column=1, sticky="ew", pady=(2,0))

    # --- Data Capture Group ---
    actions_frame = ttk.Labelframe(controls_frame, text=" Data Capture ", padding="10 5 10 10") # Reduced padding
    actions_frame.grid(row=controls_row_idx, column=0, sticky="new", pady=(0, 5)) # Reduced bottom padding
    controls_row_idx += 1
    # Configure 2 columns for actions frame to place widgets side-by-side
    actions_frame.columnconfigure(0, weight=1)
    actions_frame.columnconfigure(1, weight=1) # Give second column weight too
    action_row_idx = 0

    # Target Material Dropdown
    target_title_label = ttk.Label(actions_frame, text="Target Material:")
    # Span label across columns or place dropdown next to it? Place next to it.
    target_title_label.grid(row=action_row_idx, column=0, sticky="w", pady=(0, 1)) # Reduced pady
    action_row_idx += 1
    ...
    target_dropdown.grid(row=action_row_idx, column=0, columnspan=2, sticky="ew", pady=(0, 5)) # Span dropdown, reduced pady
    action_row_idx += 1

    # Condition Checkboxes
    condition_label = ttk.Label(actions_frame, text="Conditions:")
    condition_label.grid(row=action_row_idx, column=0, columnspan=2, sticky="w", pady=(5, 1)) # Reduced pady
    action_row_idx += 1
    coated_var = tk.IntVar(value=0)
    dilapidated_var = tk.IntVar(value=0)
    degraded_var = tk.IntVar(value=0)
    # Keep vertical for now, just reduce padding
    cb_coated = ttk.Checkbutton(actions_frame, text="Coated", variable=coated_var, onvalue=1, offvalue=0)
    cb_coated.grid(row=action_row_idx, column=0, columnspan=2, sticky="w", pady=0) # No pady
    action_row_idx += 1
    cb_dilapidated = ttk.Checkbutton(actions_frame, text="Dilapidated", variable=dilapidated_var, onvalue=1, offvalue=0)
    cb_dilapidated.grid(row=action_row_idx, column=0, columnspan=2, sticky="w", pady=0) # No pady
    action_row_idx += 1
    cb_degraded = ttk.Checkbutton(actions_frame, text="Degraded", variable=degraded_var, onvalue=1, offvalue=0)
    cb_degraded.grid(row=action_row_idx, column=0, columnspan=2, sticky="w", pady=(0, 5)) # Reduced pady
    action_row_idx += 1

    # New Object Control + Next Filename (Side-by-side)
    new_object_var = tk.IntVar(value=1)
    cb_new_object = ttk.Checkbutton(actions_frame, text="New Object?", variable=new_object_var, onvalue=1, offvalue=0, command=update_next_filename_display)
    cb_new_object.grid(row=action_row_idx, column=0, sticky="w", pady=(5, 5)) # Pad top and bottom

    next_filename_label = ttk.Label(actions_frame, text="Next: ...", style="Filename.TLabel", anchor='w')
    next_filename_label.grid(row=action_row_idx, column=1, sticky="ew", pady=(5, 5), padx=(5, 0)) # Place in col 1
    action_row_idx += 1

    # Buttons (Side-by-side)
    # No separator
    capture_button = ttk.Button(actions_frame, text="Capture & Add", command=capture_and_save_data) # Shortened text
    capture_button.grid(row=action_row_idx, column=0, sticky="ew", pady=(5, 5), padx=(0, 2)) # Pad top/bottom, space between buttons

    calibrate_button = ttk.Button(actions_frame, text="Calibrate", command=calibrate_sensors) # Shortened text
    calibrate_button.grid(row=action_row_idx, column=1, sticky="ew", pady=(5, 5), padx=(2, 0)) # Pad top/bottom, space between buttons
    action_row_idx += 1
...
def run_application():
    ...
    messagebox.showinfo("Info", "Application Ready.") # Initial message popup
