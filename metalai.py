# CODE 3.0.1 - AI Metal Classifier GUI with Results Page
...
# Version: 3.0.1 - Replaced Delta symbol (Δ) with "Delta" to fix UnicodeEncodeError on some terminals.
...
def initialize_ai():
    ...
    try:
        # Ignore specific NumPy warnings during TFLite initialization if they persist
...
def initialize_ldc1101():
    ...
    try:
        ...
        time.sleep(0.01); print("LDC1101 Configured and in Sleep Mode.")
        ldc_initialized = True; return True
    except Exception as e: print(f"Exception during LDC1101 initialization: {e}"); ldc_initialized = False; return False
...
def enable_ldc_rpmode():
    if not spi or not ldc_initialized: print("Cannot enable RP mode: SPI or LDC not initialized."); return;
...
def preprocess_input(image_pil, mag_mT, ldc_rp_delta):
    ...
    except Exception as e: print(f"ERROR: Failed during image preprocessing: {e}"); return None
    ...
    try:
        with warnings.catch_warnings(): # Suppress feature name warning
    ...
    num_features_expected = 2
    for detail in input_details:
        shape, index = detail['shape'], detail['index']
        if len(shape) == 4 and shape[1] == AI_IMG_HEIGHT and shape[2] == AI_IMG_WIDTH and shape[3] == 3: image_input_index = index
    ...
    if image_input_index == -1 or numerical_input_index == -1:
         print("Warning: Could not reliably determine input indices from shape. Trying fallback.")
         if len(input_details) == 2:
              try:
                  ...
                  else: print("Fallback failed: Dim mismatch."); return None
              except Exception as e: print(f"Fallback failed: {e}"); return None
    ...
    except Exception as e: print(f"ERROR: Failed setting input types: {e}"); return None
    return model_inputs
...
def capture_and_classify():
    ...
    global rv_image_label, rv_prediction_label, rv_confidence_label
    global rv_magnetism_label, rv_ldc_label, interpreter
    if not interpreter or not camera:
        messagebox.showerror("Error", "AI Model or Camera not ready."); return

    lv_classify_button.config(state=tk.DISABLED); window.update_idletasks()
    print("Capturing image..."); ret, frame = camera.read()
    if not ret: messagebox.showerror("Capture Error", "Failed to capture image."); lv_classify_button.config(state=tk.NORMAL); return
    img_captured_pil = Image.fromarray(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)); print("Image captured.")

    print("Reading sensors..."); avg_voltage = get_averaged_hall_voltage(num_samples=NUM_SAMPLES_CALIBRATION)
    current_mag_mT, mag_display_text = None, "N/A"
    ...
    if current_rp_val_avg is not None:
        ...
        if IDLE_RP_VALUE != 0:
            ...
            # --- Use "Delta" instead of symbol ---
            ldc_display_text = f"{current_rp_val} (Delta {delta_rp:+,})" 
        else: ldc_display_text = f"{current_rp_val} (No Cal)"
    else: ldc_display_text = "Read Error"
    # --- Use "Delta" in print statement ---
    ...
    if model_inputs is None: messagebox.showerror("AI Error", "Failed to preprocess data."); lv_classify_button.config(state=tk.NORMAL); return

    print("Running inference..."); output_data = run_inference(model_inputs)
    if output_data is None: messagebox.showerror("AI Error", "Failed during model inference."); lv_classify_button.config(state=tk.NORMAL); return
    ...
    try:
        w, h = img_captured_pil.size; aspect = h / w; img_h = int(RESULT_IMG_DISPLAY_WIDTH * aspect)
        img_disp = img_captured_pil.resize((RESULT_IMG_DISPLAY_WIDTH, img_h), Image.Resampling.LANCZOS)
        img_tk = ImageTk.PhotoImage(img_disp)
        if rv_image_label: rv_image_label.img_tk = img_tk; rv_image_label.config(image=img_tk)
    except Exception as e: print(f"Error displaying result image: {e}"); rv_image_label.config(image='', text="Img Error")
...
def calibrate_sensors():
    ...
    hall_results, hall_error = "Hall N/A", False
    if hall_sensor:
        avg_v = get_averaged_hall_voltage(num_samples=NUM_SAMPLES_CALIBRATION)
        if avg_v is not None: IDLE_VOLTAGE = avg_v; hall_results = f"Hall Idle: {IDLE_VOLTAGE:.4f} V"
        else: hall_results, hall_error, IDLE_VOLTAGE = "Hall Cal Error", True, 0
    ldc_results, ldc_error = "LDC N/A", False
    if ldc_initialized:
        avg_rp = get_averaged_rp_data(num_samples=NUM_SAMPLES_CALIBRATION)
        if avg_rp is not None: IDLE_RP_VALUE = int(avg_rp); ldc_results = f"LDC Idle: {IDLE_RP_VALUE}"
        else: ldc_results, ldc_error, IDLE_RP_VALUE = "LDC Cal Error", True, 0

    previous_filtered_mag_mT = None; print("Magnetism filter reset.")
...
def update_camera_feed():
    ...
    if camera and camera.isOpened():
        ...
        if ret:
            try:
                img = Image.fromarray(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)).resize((DISPLAY_IMG_WIDTH, DISPLAY_IMG_HEIGHT), Image.Resampling.NEAREST)
    ...
    if lv_camera_label:
        if img_tk: lv_camera_label.img_tk = img_tk; lv_camera_label.configure(image=img_tk, text="")
            ...
            if not hasattr(lv_camera_label, 'no_cam_img'):
                try: lv_camera_label.no_cam_img = ImageTk.PhotoImage(Image.new('RGB', (DISPLAY_IMG_WIDTH, DISPLAY_IMG_HEIGHT), '#BDBDBD'))
                except: lv_camera_label.no_cam_img = None
            if lv_camera_label.no_cam_img and lv_camera_label.cget("image") != str(lv_camera_label.no_cam_img): lv_camera_label.configure(image=lv_camera_label.no_cam_img, text="No Camera Feed")
...
def update_magnetism():
    ...
    avg_voltage = get_averaged_hall_voltage(); display_text = "N/A"
    if hall_sensor:
        if avg_voltage is not None:
            try:
                idle_v = IDLE_VOLTAGE if IDLE_VOLTAGE != 0 else avg_voltage
                ...
                display_text = f"{value:+.2f} {unit}" # Add sign
    ...
    if ldc_initialized:
        if avg_rp_val is not None:
            ...
            if RP_DISPLAY_BUFFER:
                buffer_avg = sum(RP_DISPLAY_BUFFER) / len(RP_DISPLAY_BUFFER); current_rp_int = int(buffer_avg)
                if IDLE_RP_VALUE != 0:
                    delta = current_rp_int - IDLE_RP_VALUE
                    # --- Use "Delta" instead of symbol ---
                    display_rp_text = f"{current_rp_int} (Delta {delta:+,})" 
...
def setup_gui():
    ...
    window = tk.Tk(); window.title("AI Metal Classifier v3.0.1"); window.geometry("1000x650")
    style = ttk.Style(); style.theme_use('clam' if 'clam' in style.theme_names() else 'default')
    title_font=tkFont.Font(family="Helvetica", size=16, weight="bold"); label_font=tkFont.Font(family="Helvetica", size=11)
    ...
    result_title_font=tkFont.Font(family="Helvetica", size=12, weight="bold"); result_value_font=tkFont.Font(family="Consolas", size=14, weight="bold")
    style.configure("TLabel",font=label_font,padding=2); style.configure("TButton",font=button_font,padding=(10, 6))
    style.configure("TLabelframe",padding=8); style.configure("TLabelframe.Label",font=tkFont.Font(family="Helvetica",size=12,weight="bold"))
    style.configure("Readout.TLabel",font=readout_font,padding=(5,1)); style.configure("ResultTitle.TLabel",font=label_font, padding=(5,2))
    ...
    ttk.Label(lv_readings_frame, text="LDC RP (Delta):").grid(row=1, column=0, sticky="w", padx=(0, 10), pady=(3,0)); lv_ldc_label = ttk.Label(lv_readings_frame, text="Init...", style="Readout.TLabel", anchor="e"); lv_ldc_label.grid(row=1, column=1, sticky="ew", pady=(3,0))
    ...
    except: placeholder_img_tk = None
    ...
    ttk.Label(rv_details_frame, text="Material:", style="ResultTitle.TLabel").grid(row=0, column=0, sticky="w", padx=5); rv_prediction_label = ttk.Label(rv_details_frame, text="...", style="ResultValue.TLabel"); rv_prediction_label.grid(row=0, column=1, sticky="ew", padx=5)
    ttk.Label(rv_details_frame, text="Confidence:", style="ResultTitle.TLabel").grid(row=1, column=0, sticky="w", padx=5); rv_confidence_label = ttk.Label(rv_details_frame, text="...", style="ResultValue.TLabel"); rv_confidence_label.grid(row=1, column=1, sticky="ew", padx=5)
    ttk.Separator(rv_details_frame, orient='horizontal').grid(row=2, column=0, columnspan=2, sticky='ew', pady=8)
    ttk.Label(rv_details_frame, text="Magnetism Used:", style="ResultTitle.TLabel").grid(row=3, column=0, sticky="w", padx=5); rv_magnetism_label = ttk.Label(rv_details_frame, text="...", style="ResultValue.TLabel"); rv_magnetism_label.grid(row=3, column=1, sticky="ew", padx=5)
    ttk.Label(rv_details_frame, text="LDC (Delta) Used:", style="ResultTitle.TLabel").grid(row=4, column=0, sticky="w", padx=5); rv_ldc_label = ttk.Label(rv_details_frame, text="...", style="ResultValue.TLabel"); rv_ldc_label.grid(row=4, column=1, sticky="ew", padx=5)
    rv_classify_another_button = ttk.Button(rv_content_frame, text="<< Classify Another", command=show_live_view); rv_classify_another_button.grid(row=3, column=0, columnspan=2, pady=(10, 5))

    # --- Initial GUI State ---
    clear_results_display() # Init results widgets
    show_live_view() # Show live view first
...
if __name__ == '__main__':
    ...
    try:
        ...
        if hardware_ok: cleanup_resources()
