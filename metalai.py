...
def ldc_write_register(reg_addr, value):
    ...
    try:
        ...
        try: GPIO.output(CS_PIN, GPIO.HIGH) # Ensure CS is high on error
        except: pass
...
def initialize_ldc1101():
    ...
    try:
        ...
        # Use assert for critical checks like chip ID during init
        ...
        ldc_write_register(START_CONFIG_REG, SLEEP_MODE) # Start in sleep
        time.sleep(0.01) # Short delay for settings to take effect
        print("LDC1101 Configured.")
        ldc_initialized = True
        return True
    except Exception as e:
        print(f"LDC Init Exception: {e}")
...
def enable_ldc_powermode(mode):
    ...
    time.sleep(0.01) # Short delay after mode change
...
def get_averaged_hall_voltage(num_samples=NUM_SAMPLES_PER_UPDATE):
    ...
    for _ in range(num_samples):
        ...
        except Exception: return None # Exit early if read fails
...
def preprocess_input(image_pil, mag_mT, ldc_rp_delta):
    ...
    # Fallback if indices not found by shape matching (e.g., if model uses generic shapes like [-1, -1])
    if image_input_index == -1 or numerical_input_index == -1:
         print("Warning: Could not determine input indices from shape. Trying fallback based on order.")
         if len(input_details) == 2:
              try:
                  # Simple assumption: one is 4D (image), one is 2D (numerical)
                  ...
                  if len(i0_shape) == 4 and len(i1_shape) == 2:
                       ...
                       print("Fallback failed: Dimensions mismatch (not 4D and 2D).")
                       return None
              except Exception as e:
                    print(f"Fallback identification failed: {e}")
                    return None
         else:
    ...
    try:
        # Get expected dtypes from model details
...
def run_inference(model_inputs):
    ...
    if interpreter is None or model_inputs is None:
        print("ERROR: Interpreter or input data missing.")
        return None
    try:
        # Set input tensors from the dictionary
        for index, data in model_inputs.items():
             ...
             interpreter.set_tensor(index, data) # Set the tensor data
        ...
        # Optionally print full traceback for difficult errors
        # import traceback
        # traceback.print_exc()
        return None
...
def postprocess_output(output_data):
    ...
    if output_data is None or not loaded_labels:
        return "Error", 0.0
    try:
        # Assuming output is shape (1, num_classes) with probabilities/logits
        probabilities = output_data[0]
        print(f"Debug Postprocess: Raw output tensor: {probabilities}") # <<< DEBUG PRINT
        # --- Potential Fix: Apply Softmax if output is logits ---
        # If you suspect the model outputs logits, uncomment the next 3 lines:
        # if np.min(probabilities) < 0 or np.max(probabilities) > 1 or not np.isclose(np.sum(probabilities), 1.0):
        #     print("Debug Postprocess: Applying Softmax to output.")
        #     probabilities = np.exp(probabilities - np.max(probabilities)) / np.sum(np.exp(probabilities - np.max(probabilities)))
        # ---------------------------------------------------------
        predicted_index = np.argmax(probabilities)
        predicted_label = loaded_labels[predicted_index]
        confidence = float(probabilities[predicted_index]) # Cast to standard python float
...
def capture_and_classify():
    ...
    if not interpreter or not camera:
        messagebox.showerror("Error", "AI Model or Camera not ready.")
        return

    lv_classify_button.config(state=tk.DISABLED)
    window.update_idletasks() # Ensure button visually disables
    ...
    # Get sensor readings using calibration sample count for stability
    ...
    if avg_voltage is not None:
        ...
        except Exception: # Catch potential division by zero or other math errors
            mag_display_text = "Calc Error"
        if IDLE_VOLTAGE == 0: mag_display_text += " (No Cal)" # Append if uncalibrated
    ...
    if current_rp_val_avg is not None:
        ...
        if IDLE_RP_VALUE != 0:
            ...
            ldc_display_text = f"{current_rp_val} (Delta {delta_rp:+,})" # Use "Delta" not symbol
        else:
            ldc_display_text = f"{current_rp_val} (No Cal)"
    else:
        ldc_display_text = "Read Error"
    # Print sensor readings used for this classification
    ...
    if model_inputs is None:
        messagebox.showerror("AI Error", "Failed to preprocess data for model.")
        lv_classify_button.config(state=tk.NORMAL) # Re-enable on failure
        return
    ...
    # Update the results page widgets
    print("Updating results display...")
    try: # Update image
        w, h = img_captured_pil.size
        aspect = h / w
        img_h = int(RESULT_IMG_DISPLAY_WIDTH * aspect)
    ...
    show_results_view()
    # Button will be re-enabled by show_live_view when user clicks "Classify Another"
...
def calibrate_sensors():
    ...
    if hall_sensor:
        ...
        if avg_v is not None:
            ...
            hall_results, hall_error, IDLE_VOLTAGE = "Hall Cal Error", True, 0.0

    # Perform LDC calibration
    ...
    if ldc_initialized:
        ...
        if avg_rp is not None:
            ...
            ldc_results, ldc_error, IDLE_RP_VALUE = "LDC Cal Error", True, 0

    previous_filtered_mag_mT = None # Reset filter state
    ...
    if camera and camera.isOpened():
        ...
        if ret:
            try:
                ...
                img.thumbnail((DISPLAY_IMG_WIDTH, DISPLAY_IMG_HEIGHT), Image.Resampling.NEAREST) # Resize keeping aspect ratio
                img_tk = ImageTk.PhotoImage(img)
            except Exception as e:
                print(f"Cam frame proc error: {e}") # Log error

    if lv_camera_label: # Check if widget exists
        if img_tk:
            lv_camera_label.img_tk = img_tk # Keep reference
            lv_camera_label.configure(image=img_tk, text="")
        else: # Handle missing camera feed
            if not hasattr(lv_camera_label, 'no_cam_img'): # Create placeholder once
                try:
                    ph = Image.new('RGB', (DISPLAY_IMG_WIDTH // 2, DISPLAY_IMG_HEIGHT // 2), '#BDBDBD') # Smaller placeholder
                    lv_camera_label.no_cam_img = ImageTk.PhotoImage(ph)
                except Exception: lv_camera_label.no_cam_img = None # Handle tkinter errors
            # Display placeholder if available and needed
    ...
    if hall_sensor:
        if avg_voltage is not None:
            try:
                ...
                if IDLE_VOLTAGE == 0: display_text += " (No Cal)" # Indicate if not calibrated
            except Exception: # Catch division by zero or other errors
                display_text = "Error"
...
def update_ldc_reading():
    ...
    if ldc_initialized:
        if avg_rp_val is not None:
            RP_DISPLAY_BUFFER.append(avg_rp_val) # Add to buffer
            if RP_DISPLAY_BUFFER: # Ensure buffer is not empty
                ...
                if IDLE_RP_VALUE != 0: # If calibrated
                    delta = current_rp_int - IDLE_RP_VALUE
                    display_rp_text = f"{current_rp_int} (Delta {delta:+,})" # Use "Delta"
                else:
                    display_rp_text = f"{current_rp_int} (No Cal)"
            else:
                display_rp_text = "..." # Buffer initializing
        else:
...
# ======================
def setup_gui():
    # Define globals used in this function
    ...
    # --- Window and Style ---
    window = tk.Tk()
    window.title("AI Metal Classifier v3.0.2")
    ...
    style.configure("ResultTitle.TLabel", font=label_font, padding=(5, 2)) # Result labels (e.g., "Material:")
    style.configure("ResultValue.TLabel", font=result_value_font, padding=(5, 1), anchor=tk.E) # Result values (right aligned)
    ...
    lv_controls_frame.grid(row=0, column=1, sticky="nsew"); lv_controls_frame.columnconfigure(0, weight=1)
    # Readings Sub-frame
    ...
    # Inner frame for easier layout management
    rv_content_frame = ttk.Frame(results_view_frame)
    rv_content_frame.grid(row=0, column=0, sticky="n") # Place at top-center
    # Title
    ...
    rv_details_frame.grid(row=2, column=0, columnspan=2, pady=(0, 15)); rv_details_frame.columnconfigure(1, weight=1) # Value column expands
    ...
    rv_prediction_label = ttk.Label(rv_details_frame, text="...", style="ResultValue.TLabel"); rv_prediction_label.grid(row=res_row, column=1, sticky="ew", padx=5); res_row+=1
    ttk.Label(rv_details_frame, text="Confidence:", style="ResultTitle.TLabel").grid(row=res_row, column=0, sticky="w", padx=5)
    rv_confidence_label = ttk.Label(rv_details_frame, text="...", style="ResultValue.TLabel"); rv_confidence_label.grid(row=res_row, column=1, sticky="ew", padx=5); res_row+=1
...
def run_application():
    global window
    setup_gui() # Setup all GUI elements
    ...
    if not interpreter: # Disable button if AI failed
        if lv_classify_button: lv_classify_button.config(state=tk.DISABLED)
...
if __name__ == '__main__':
    ...
    try:
        ...
        try: # Try to show error in GUI if possible
        ...
        # Cleanup hardware resources only if initialization was attempted
        if hardware_ok:
             cleanup_resources()
