# CODE 3.0.23 - AI Metal Classifier GUI with Selectable Sensor Modes
# Description: Displays live sensor data and camera feed.
#              - A global variable `ACTIVE_SENSOR_MODE` allows choosing the active sensor(s).
...
# Version: 3.0.23 - ADDED: Integer-based sensor selection for easier mode switching.
#                  -        0 = Multimodal (Original)
#                  -        1 = Vision Only
#                  -        2 = Magnetism Only
#                  -        3 = LDC/Conductivity Only
#                  - MODIFIED: Hardware initialization and AI preprocessing now respect the selected mode.
#                  -           Disabled sensors have their inputs replaced with neutral baseline values.
...
# ==================================
# NEW: Select the active sensor mode using an integer.
# 0 = Multimodal (All sensors active)
# 1 = Vision Only
# 2 = Magnetism Only
# 3 = LDC/Conductivity Only
ACTIVE_SENSOR_MODE = 1  # <<< CHANGE THIS VALUE TO SELECT THE SENSOR MODE
...
def initialize_hardware():
    ...
    # Map integer mode to descriptive string for logging
    mode_map = {0: "Multimodal", 1: "Vision Only", 2: "Magnetism Only", 3: "LDC/Conductivity Only"}
    mode_string = mode_map.get(ACTIVE_SENSOR_MODE, "Unknown")

    print("\n--- Initializing Hardware ---")
    print(f"--- ACTIVE SENSOR MODE: {ACTIVE_SENSOR_MODE} ({mode_string}) ---")


    # --- Camera Initialization (Always initialized for live feed) ---
    ...
    # --- I2C/ADS1115 Initialization (Only for Magnetism or Multimodal) ---
    if ACTIVE_SENSOR_MODE in [0, 2]: # 0=Multimodal, 2=Magnetism
        if I2C_ENABLED:
        ...
        print("Skipping I2C/ADS1115 setup (disabled by ACTIVE_SENSOR_MODE).")
    ...
    # --- SPI/LDC1101 Initialization (Only for LDC or Multimodal) ---
    if ACTIVE_SENSOR_MODE in [0, 3]: # 0=Multimodal, 3=LDC
        ...
        print("Skipping SPI/LDC1101 setup (disabled by ACTIVE_SENSOR_MODE).")
...
def preprocess_input(image_pil, mag_mT, ldc_rp_raw):
    global numerical_scaler, input_details, interpreter, IDLE_RP_VALUE
    ...
    try:
        # The AI model expects a specific image size.
        # If we passed a neutral gray image, it's already the right size.
        # If we passed a real image, it gets resized here.
    ...
    # --- NEW: Zero out inactive sensor data based on ACTIVE_SENSOR_MODE ---
    mode_map = {0: "Multimodal", 1: "Vision Only", 2: "Magnetism Only", 3: "LDC/Conductivity Only"}
    mode_string = mode_map.get(ACTIVE_SENSOR_MODE, "Unknown")
    print(f"DEBUG Preprocess: Active mode is {ACTIVE_SENSOR_MODE} ('{mode_string}'). Adjusting numerical inputs.")

    if ACTIVE_SENSOR_MODE == 1: # Vision Only
        mag_mT_val = 0.0
        ldc_rp_raw_val = float(IDLE_RP_VALUE) # Use calibrated idle value as baseline
        print(f"DEBUG Preprocess: Vision mode. Using mag=0.0, ldc={ldc_rp_raw_val} (idle)")
    elif ACTIVE_SENSOR_MODE == 2: # Magnetism Only
        ldc_rp_raw_val = float(IDLE_RP_VALUE) # Use calibrated idle value for LDC
        print(f"DEBUG Preprocess: Magnetism mode. Using ldc={ldc_rp_raw_val} (idle)")
    elif ACTIVE_SENSOR_MODE == 3: # LDC Only
        mag_mT_val = 0.0 # Use 0 for magnetism
        print(f"DEBUG Preprocess: LDC mode. Using mag=0.0")
    # Mode 0 (Multimodal) uses the original values passed to the function.

    numerical_features = np.array([[mag_mT_val, ldc_rp_raw_val]], dtype=np.float32)
    print(f"DEBUG Preprocess: Raw numerical features for AI: {numerical_features}")
...
def capture_and_classify():
    ...
    # 1. Capture the real camera frame for display purposes
    ...
    try:
        # This image will be shown in the results view and screenshots
        img_for_display = Image.fromarray(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
    except Exception as e:
    ...
    # 2. Determine which image to send to the AI model
    if ACTIVE_SENSOR_MODE in [2, 3]: # 2=Magnetism, 3=LDC
        # For non-visual modes, create a neutral gray image to not influence the model
        print(f"DEBUG: Mode is {ACTIVE_SENSOR_MODE}, creating neutral gray image for AI.")
        img_for_ai = Image.new('RGB', (AI_IMG_WIDTH, AI_IMG_HEIGHT), (128, 128, 128))
    else: # 0=Multimodal, 1=Vision
        # For vision or multimodal, use the actual captured image
        img_for_ai = img_for_display
    ...
    if sensor_warning and ACTIVE_SENSOR_MODE not in [1]: # Not Vision Only
        print("WARNING: Sensor issues may affect classification.")

    # Pass the AI-specific image and the raw sensor values to be processed
    model_inputs = preprocess_input(img_for_ai, current_mag_mT, current_rp_raw)
    ...
    if save_output_var and save_output_var.get() == 1:
        # Always save the screenshot with the REAL image
        save_result_screenshot(img_for_display, predicted_label, confidence, mag_display_text, ldc_display_text)

    send_sorting_signal(predicted_label)

    # Update Results Display with the REAL image
    if rv_image_label:
        try:
            w, h_img = img_for_display.size; aspect = h_img/w if w>0 else 0.75
            display_h = int(RESULT_IMG_DISPLAY_WIDTH * aspect) if aspect > 0 else int(RESULT_IMG_DISPLAY_WIDTH * 0.75)
            img_disp = img_for_display.resize((RESULT_IMG_DISPLAY_WIDTH, max(1, display_h)), Image.Resampling.LANCZOS)
...
def setup_gui():
    ...
    window = tk.Tk()
    window.title("AI Metal Classifier v3.0.23 (RPi - Selectable Sensor)")
    ...
    # Display the current sensor mode in the GUI
    mode_map = {0: "Multimodal", 1: "Vision Only", 2: "Magnetism Only", 3: "LDC/Conductivity Only"}
    mode_string = mode_map.get(ACTIVE_SENSOR_MODE, "Unknown")
    mode_color = "#006400" if ACTIVE_SENSOR_MODE == 0 else "#B22222" # Green for multimodal, Red for single
    ttk.Label(
        lv_actions_frame,
        text=f"Mode: {mode_string}",
