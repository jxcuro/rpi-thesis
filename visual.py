...
# MODIFIED: Changed to visual-only model. Scaler is no longer needed.
MODEL_FILENAME = "visual_model.tflite"
...
# === AI Model Setup ======
# =========================
# MODIFIED: Removed scaler loading and changed model input check to 1.
def initialize_ai():
    global interpreter, input_details, output_details, loaded_labels
    ...
    if ai_ready:
        ...
        try:
            ...
            if len(input_details) != 1:
                print(f"ERROR: Model should have 1 input (visual), but found {len(input_details)}.")
                ai_ready = False
            if output_details and output_details[0]['shape'][-1] != len(loaded_labels):
                print(f"ERROR: Model output size ({output_details[0]['shape'][-1]}) != labels ({len(loaded_labels)})."); ai_ready = False
...
# === AI Processing ========
# ==========================
# MODIFIED: Reworked to only process the image input and ignore sensor values.
def preprocess_input(image_pil, mag_mT, ldc_rp_raw):
    # NOTE: mag_mT and ldc_rp_raw are ignored for this visual-only model.
    global input_details, interpreter
    print("\n--- Preprocessing Input for AI (Visual Only) ---")
    if interpreter is None or input_details is None:
        print("ERROR: AI Model not initialized."); return None

    # --- 1. Preprocess Image Input ---
    try:
    ...
    # --- 2. Prepare Model Inputs (Visual Only) ---
    # The model is expected to have only one input: the image.
    image_input_detail = input_details[0]
    image_input_index = image_input_detail['index']
    image_input_dtype = image_input_detail['dtype']

    # Convert image data to the type expected by the model
    final_image_input = image_input.astype(image_input_dtype)

    # Handle quantization (if model expects uint8, rescale from [0,1] to [0,255])
    ...
    # The final input for the model is a dictionary with one entry
    model_inputs = { image_input_index: final_image_input }
