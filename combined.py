...
# Version: v1.2 - CLAHE Glare Reduction, LDC Smoothing, GUI Layout Fix
...
LDC_DISPLAY_BUFFER_SIZE = 5 # Number of readings for display moving average
...
# NOTE: Consistent positioning of the sample relative to the LDC coil
# is CRITICAL for repeatable measurements. Use a jig/fixture if possible.
...
# << Added delta_rp to header
METADATA_HEADER = ['image_path', 'magnetism_mT', 'ldc_rp', 'delta_rp', 'target_material']
...
clahe_processor = None # CLAHE object

# --- Global State ---
# << Buffer for smoothing displayed LDC value
...
# === CSV Handling =========
# =========================
# << Modified to include delta_rp
def append_metadata(image_rel_path, mag_mT, rp_value, delta_rp, target):
    ...
    try:
        with open(METADATA_PATH, 'a', newline='') as csvfile:
            ...
            writer.writerow([image_rel_path, mag_mT_str, rp_value_str, delta_rp_str, target])
        return True
...
def enhance_image_contrast(frame_rgb):
    ...
    if clahe_processor is None: return frame_rgb # Return original if CLAHE failed init
    try:
        # Convert to LAB color space
        lab = cv2.cvtColor(frame_rgb, cv2.COLOR_RGB2LAB)
        l, a, b = cv2.split(lab)
        # Apply CLAHE to the L-channel
        l_clahe = clahe_processor.apply(l)
        # Merge the CLAHE enhanced L-channel back
        lab_clahe = cv2.merge((l_clahe, a, b))
        # Convert back to RGB
        frame_rgb_enhanced = cv2.cvtColor(lab_clahe, cv2.COLOR_LAB2RGB)
        return frame_rgb_enhanced
...
def capture_and_save_data():
    global feedback_label, capture_button, window, target_var, IDLE_RP_VALUE # Need idle RP
    ...
    img_to_save = Image.fromarray(frame_enhanced_rgb) # Save enhanced version
    ...
    # Get Averaged Sensor Readings (using more samples for capture)
    ...
    if current_rp_val is not None:
        ...
        # << Calculate Delta RP
        if IDLE_RP_VALUE is not None and IDLE_RP_VALUE != 0: # Avoid division by zero if idle is 0? No, just subtraction. Check if calibrated.
    ...
    # << Append Metadata including delta_rp
    meta_success = append_metadata(image_relative_path, current_mag_mT, current_rp_val, delta_rp, selected_target)
...
def update_camera_feed():
    ...
    if camera:
        ...
        if ret:
            ...
            frame_enhanced_rgb = enhance_image_contrast(frame_rgb)
            img = Image.fromarray(frame_enhanced_rgb).resize((IMG_WIDTH, IMG_HEIGHT))
...
def update_magnetism():
    # (Function remains the same, uses averaging)
    global magnetism_label, window
    avg_voltage = get_averaged_hall_voltage()
    if avg_voltage is not None:
        try:
            adjusted_voltage = avg_voltage - IDLE_VOLTAGE; magnetism_mT = adjusted_voltage / SENSITIVITY_V_PER_MILLITESLA
...
def update_ldc_reading():
    ...
    if not ldc_initialized:
        ...
        # Calculate the moving average from the buffer
        ...
        # Error reading sensor, but keep showing previous buffer avg? Or show Error?
        # Option 1: Show last known smoothed value (might be misleading if sensor died)
        # if len(RP_DISPLAY_BUFFER) > 0:
        #     moving_avg_rp = sum(RP_DISPLAY_BUFFER) / len(RP_DISPLAY_BUFFER)
        #     display_rp_text = f"{int(moving_avg_rp)} (Error)"
        # else:
        #     display_rp_text = "Error"
        # Option 2: Clear buffer and show Error (more direct)
...
# ======================
def setup_gui():
    # << Added anchor='e' to readout labels
    ...
    window = tk.Tk(); window.title("Sensor Data Acquisition Tool v1.2"); window.geometry("1100x700")
    ...
    readout_font = tkFont.Font(family="Consolas", size=14, weight="bold") # Monospaced helps alignment
    ...
    style.configure("Readout.TLabel", font=readout_font, padding=(5, 2)) # Padding for readout labels
    ...
    readings_frame.columnconfigure(0, weight=0); readings_frame.columnconfigure(1, weight=1) # Value column expands
    ttk.Label(readings_frame, text="Magnetism:").grid(row=0, column=0, sticky="w", padx=(0, 10))
    # << Use anchor='e' and sticky='ew'
