...
# Version: v1.3 - Checkboxes, Image Resize, Sequential Naming, GUI Fixes
...
import time
import os
import uuid # Keep for fallback ID if needed? No, using counter.
import csv
...
# --- Camera ---
CAMERA_INDEX = 0
DISPLAY_IMG_WIDTH = 640 # Size for GUI display
...
FILENAME_PREFIX = "data_" # << New file prefix
FILENAME_PADDING = 4 # << Number of digits (e.g., 0001)
...
data_counter = 1 # << Counter for sequential file naming
...
def initialize_global_state():
    ...
    if not setup_project_directory():
         ...
         # Consider raising an exception or using sys.exit()
...
def initialize_hardware():
    # (Function remains the same, initializes clahe)
    ...
    try:
        with open(METADATA_PATH, 'a', newline='') as csvfile:
            ...
            writer.writerow([
                ...
                is_coated, is_dilapidated, is_degraded # Add the 0/1 flags
            ])
        return True
...
def capture_and_save_data():
    # << Major changes: sequential naming, resize, get checkbox values
    ...
    global data_counter # Access and increment the counter
    ...
    try:
        img_resized_for_save = img_enhanced.resize(
            (SAVE_IMG_WIDTH, SAVE_IMG_HEIGHT),
        ...
        feedback_label.config(text=f"Error resizing image: {e}", foreground="#E53935")
    ...
    if current_rp_val is not None:
        ...
        if IDLE_RP_VALUE != 0: delta_rp = current_rp_val - IDLE_RP_VALUE # Check if calibrated?

    # Get Target and Checkbox Values
    selected_target = target_var.get()
    is_coated = coated_var.get() # 0 or 1
    is_dilapidated = dilapidated_var.get() # 0 or 1
    is_degraded = degraded_var.get() # 0 or 1
    ...
    # Append Metadata (including new fields)
    meta_success = append_metadata(
        image_filename, # Save filename, not relative path for simplicity now
        current_mag_mT,
        current_rp_val,
        delta_rp,
        selected_target,
    ...
    if meta_success:
        ...
        # Consider deleting the image file if metadata failed?
...
def update_camera_feed():
    ...
    window.after(50, update_camera_feed) # Keep camera responsive

def update_magnetism():
    # (Remains the same, updates GUI at GUI_UPDATE_INTERVAL_MS)
...
def update_ldc_reading():
    # (Uses moving average buffer, updates GUI at GUI_UPDATE_INTERVAL_MS)
...
def setup_gui():
    ...
    check_font = tkFont.Font(family="Helvetica", size=10) # Font for checkboxes
    ...
    style.configure("TCheckbutton", font=check_font, padding=3) # Style checkbuttons
    ...
    feedback_label = ttk.Label(controls_frame, text="", style="Feedback.TLabel", anchor="w", wraplength=300) # Wraplength helps control width
    feedback_label.grid(row=feedback_label_row, column=0, sticky="ew", pady=(0, 10))
    # << Set minimum height for feedback row (adjust value based on font size * ~2.5 lines)
    controls_frame.rowconfigure(feedback_label_row, minsize=int(feedback_font.metrics('linespace') * 2.5))
    row_idx += 1

    # Sensor Readings Group (remains same)
    ...
    magnetism_label = ttk.Label(readings_frame, text="Init...", style="Readout.TLabel", anchor="e") # Anchor East
    ...
    ldc_label = ttk.Label(readings_frame, text="Init...", style="Readout.TLabel", anchor="e") # Anchor East
    ...
    action_row_idx = 0 # Internal row counter for actions_frame
    ...
    cb_degraded.grid(row=action_row_idx, column=0, sticky="w", pady=(0, 10)); action_row_idx += 1 # Add padding after last checkbox
...
# ==========================
def run_application():
    global window # Ensure window is accessible
    if not initialize_global_state(): return # Setup dirs and counter first
    setup_gui()
    # Set initial GUI states based on hardware init
